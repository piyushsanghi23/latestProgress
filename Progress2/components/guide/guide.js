function slide_1(){
    document.getElementById('guide_1').style.display = 'none';
    document.getElementById('guide_2').style.display = 'block';
}
function slide_2(){
    document.getElementById('guide_2').style.display = 'none';
    document.getElementById('guide_3').style.display = 'block';
}
function slide_2B(){
    document.getElementById('guide_2').style.display = 'none';
    document.getElementById('guide_1').style.display = 'block';
}
function slide_3(){
    document.getElementById('guide_3').style.display = 'none';
    document.getElementById('guide_4').style.display = 'block';
}
function slide_3B(){
    document.getElementById('guide_3').style.display = 'none';
    document.getElementById('guide_2').style.display = 'block';
}

function slide_4B(){
    document.getElementById('guide_4').style.display = 'none';
    document.getElementById('guide_3').style.display = 'block';
}
function restart(){
    
    document.getElementById('guide_1').style.display = 'block';
    document.getElementById('guide_2').style.display = 'none';
    document.getElementById('guide_3').style.display = 'none';
    document.getElementById('guide_4').style.display = 'none';
    
}

        